...
export function ArticleRenderer({ article }: { article: Article }) {
  ...
  return <>
    <JsonLd data={{ "@context": "https://schema.org", "@graph": [
      ...
      { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: `${origin}/${article.locale}/` }, { "@type": "ListItem", position: 2, name: article.contentType === "blog" ? "Blog" : "News", item: `${origin}/${article.locale}/${article.contentType === "blog" ? "blog" : "news"}` }, { "@type": "ListItem", position: 3, name: article.title, item: `${origin}${path}` }] },
      ...(faqSchema ? [faqSchema] : []),
    ] }} />
    <article className="article"><div className="article-head"><nav aria-label="Breadcrumb"><Link href={`/${article.locale}/`}>Home</Link> / <Link href={article.contentType === "blog" ? `/${article.locale}/blog` : `/${article.locale}/news`}>{article.contentType === "blog" ? "Blog" : "News"}</Link></nav><span className="eyebrow">{article.contentType.replace("_", " ")}</span><h1>{article.title}</h1><p className="article-summary">{article.summary}</p><div className="article-meta"><time dateTime={article.publishedAt}>{article.publishedAt}</time>{article.modifiedAt && article.modifiedAt !== article.publishedAt ? <time dateTime={article.modifiedAt}>Updated {article.modifiedAt}</time> : null}<Link href={article.author.profilePath || `/${article.locale}/editorial-policy`}>{article.author.name}</Link></div></div>
      ...
      <div className="article-layout"><aside className="article-toc"><strong>On this page</strong>{article.sections.filter((section) => section.level === 2).map((section) => <a key={section.heading} href={`#${slug(section.heading)}`}>{section.heading}</a>)}</aside><div className="article-copy">
