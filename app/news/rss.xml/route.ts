...
export async function GET() {
  const articles = (await listPublishedNews("en"))
    .filter((article) => Boolean(article.publishedAt) && (article.contentType === "industry_news" || article.contentType === "company_news"))
    .slice(0, 50);
  const items = articles.map((article) => {
    const url = `${origin}${articlePath(article)}`;
    const source = article.sources[0];
    return `<item><title>${xml(article.title)}</title><link>${xml(url)}</link><guid isPermaLink="true">${xml(url)}</guid><description>${xml(article.summary)}</description><pubDate>${new Date(article.publishedAt!).toUTCString()}</pubDate><author>BZMAGNET Editorial Team</author>${source ? `<source url="${xml(source.url)}">${xml(source.publisher)}</source>` : ""}</item>`;
  }).join("");
  ...
  return new Response(body, { headers: { "content-type": "application/rss+xml; charset=utf-8", "cache-control": "no-store, max-age=0" } });
}
