...
const categoryImages=["category-conveyor-context.png","category-mineral-bulk-context.png","category-recycling-sorting-context.png","category-process-flow-context.png"];
...
export default async function Home({params}:{params:Promise<{locale:string}>}){
  ...
  const schemas=[{"@context":"https://schema.org","@type":"WebSite",name:siteConfig.name,url:origin,inLanguage:locale},{"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:c.home,url:`${origin}/${locale}/`}]}];
  return <>{schemas.map((data,index)=><JsonLd key={index} data={data}/>)}
    ...
    <section className="home-section"><div className="shell quick-inquiry"><div><span className="eyebrow">Quick Inquiry</span><h2>Tell us what you are processing</h2><p>Start with a few project details. You can add full process information in the RFQ form.</p></div><InquiryForm variant="quick" locale={locale}/></div></section>
