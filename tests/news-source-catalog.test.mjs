...
test("repeat feed scans preserve the candidate identity and linked article", () => {
  ...
  for (const token of ["source_fingerprint", "current.id", "current.article_id", "current.candidate.evidence", "articleId: row.article_id", "jsonb_build_object", "status = 'verified' and article_id is null"]) assert.ok(store.includes(token));
});
