...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: { absolute: `${copy.newsHeading} | BZMAGNET` }, description: copy.industryDescription, alternates: { ...alternates(locale, "news"), types: { "application/rss+xml": `${origin}/news/rss.xml` } } };
}
...
export default async function News({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const copy=homeCopy[locale];
  const articleCopy=uiCopy[locale].article;
